...
export default function Shop() {
  ...
  return (
    <div data-aos="fade-up" className="min-h-screen mt-15 px-4 py-12">
      <h1 className="text-4xl font-bold text-center text-amber-900 mb-6">{t("shopAllProducts")}</h1>
      <p className="text-center text-gray-600 mb-10">{t("browseOurCollection")}</p>
      <a
      ...
      <div data-aos="fade-up" className="grid grid-cols-1 md:grid-cols-3 gap-6 max-w-6xl mx-auto">
        {products.map((product: Product) => {
          const isShown = cartState[product.id]?.showControls || false;
          const { rating, count } = useRatings(product.title);

          return (
            <div data-aos="fade-up" key={product.id} className="bg-white rounded-xl shadow-md overflow-hidden">
