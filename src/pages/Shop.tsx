...
export default function Shop() {
  ...
  const handleIncrease = (p: Product) => {
    setCartState((prev) => {
      const qty = (prev[p.id]?.quantity || 0) + 1;
      addToCart({ ...p, quantity: qty });
      return { ...prev, [p.id]: { showControls: true, quantity: qty } };
    });
  };
  ...
  return (
    <div className="min-h-screen mt-15 px-4 py-12">
      ...
      <p className="text-center text-gray-600 mb-10">
        Browse through our elegant and modest collection of clothing.
      </p>
          ...
          return (
            <div key={product.id} className="bg-white rounded-xl shadow-md overflow-hidden">
              ...
              <div className="p-4 text-center">
                ...
                {!isShown ? (
                  <button
                    onClick={() => handleAddFirstTime(product)}
                    className="mt-4 text-amber-900 px-4 py-2 rounded hover:bg-amber-800 flex items-center justify-center gap-2 mx-auto"
                  >
<CartDrawer />
                  </button>
                ) : (
